...
// AUTOSTARTING
/* List of programs to start automatically during startup only */
static const char *const autostart[] = {
    "sh", "-c", "xrandr -s 1920x1080", NULL,
    "nm-applet", NULL,
    "xbanish", NULL,
    "sh", "-c", "feh --bg-fill ~/Pictures/backgrounds/fate-saberAlter.jpg", NULL,
	NULL /* terminate */
};

/* 
 * List of programs to start automatically during a restart only.
 * These should usually be short scripts that perform specific operations.
 */
...
static const unsigned int gappih = MAIN_GAPP; /* horiz inner gap between windows */
static const unsigned int gappiv = MAIN_GAPP; /* vert inner gap between windows */
static const unsigned int gappoh = MAIN_GAPP; /* horiz outer gap between windows and screen edge */
static const unsigned int gappov = MAIN_GAPP; /* vert outer gap between windows and screen edge */
static const unsigned int gappfl = MAIN_GAPP; /* gap between floating windows (when relevant) */
...
static const char *spcmd_w[] = {"w", "alacritty", "-n", "spterm (w)", "-g", "120x34", NULL };
...
static Key keys[] = {
	...
	{ KeyPress, MODKEY, XK_p,      spawn, {.v = launcher } }, // spawn dmenu for launching other programs
	{ KeyPress, MODKEY, XK_Return, spawn, {.v = termcmd } },  // spawn a terminal

	{ KeyPress, MODKEY|Ctrl, XK_b, togglebar, {0} }, // toggles the display of the bar(s) on the current monitor
	...
	{ KeyPress, MODKEY|Ctrl, XK_Return, zoom, {0} }, // moves the currently focused window to/from the master area (for tiled layouts)
