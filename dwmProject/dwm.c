...
createmon(void)
{
  ...
  /* bail out if the number of monitor exceeds the number of tags */
  ...
  /* reassing all tags to monitors since there's currently no free tag for the new monitor */ 
  if (i >= LENGTH(tags))
...
drawbar(Monitor *m)
{
    ...
    if (m == selmon) { /* status is only drawn on selected monitor */
        ...
        /* I changed this for trayer. Keep 0 and just apply half the patch xd */
...
runautostart(void)
{
    ...
    if (pid == 0) { // child
        ...
        snprintf(autostart_path, sizeof(autostart_path), "%s/.config/dwm/autostart.sh", home);

        // Executing
        char *args[] = {"/usr/bin/env", "bash", autostart_path, NULL};
...
sigchld(int unused)
{
	if (signal(SIGCHLD, sigchld) == SIG_ERR)
		die("cant't install SIGCHLD handler:");
