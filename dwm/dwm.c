...
#define ISVISIBLE(C, M) ((ISVISIBLEONTAG(C, C->mon->tagset[C->mon->seltags]) & M->tagset[M->seltags]))
...
runAutostart(void)
{
    int status = system("sh ~/.config/dwm/autostart.sh &");
    if (status != 0)
        fprintf(stderr, "Autostart script failed or missing\n");
}
...
toggletag(const Arg *arg)
{
	...
	if (newtags) {
        ...
        for (m = mons; m; m = m->next)
            if (m != selmon && newtags & m->tagset[m->seltags]) return;
